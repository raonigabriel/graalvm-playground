...
LABEL org.label-schema.name="graalvm-playground" \
      ...
      org.label-schema.version="1.0.0-rc9"
...
RUN apt-get update && \
    ...
    curl -L https://github.com/oracle/graal/releases/download/vm-1.0.0-rc9/graalvm-ce-1.0.0-rc9-linux-amd64.tar.gz -o /tmp/graalvm.tar.gz && \
    tar -zxvf /tmp/graalvm.tar.gz -C /tmp && \
    mv /tmp/graalvm-ce-1.0.0-rc9 /opt/graalvm && \
