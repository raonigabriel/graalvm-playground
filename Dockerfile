...
LABEL org.label-schema.name="graalvm-playground" \
      ...
      org.label-schema.version="19.0.0"
...
RUN apt-get update && \
    ...
    curl -L https://github.com/oracle/graal/releases/download/vm-19.0.0/graalvm-ce-linux-amd64-19.0.0.tar.gz -o /tmp/graalvm.tar.gz && \
    tar -zxvf /tmp/graalvm.tar.gz -C /tmp && \
    mv /tmp/graalvm-ce-19.0.0 /opt/graalvm && \
