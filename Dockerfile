...
LABEL org.label-schema.name="graalvm-playground" \
      ...
      org.label-schema.version="19.2.0"
...
RUN apt-get update && \
    ...
    curl http://mirror.nbtelecom.com.br/apache/maven/maven-3/3.6.2/binaries/apache-maven-3.6.2-bin.tar.gz -o /tmp/maven.tar.gz && \
    tar -zxvf /tmp/maven.tar.gz -C /tmp && \
    mv /tmp/apache-maven-3.6.2 /opt/apache-maven && \
    curl -L https://github.com/oracle/graal/releases/download/vm-19.2.0/graalvm-ce-linux-amd64-19.2.0.tar.gz -o /tmp/graalvm.tar.gz && \
    tar -zxvf /tmp/graalvm.tar.gz -C /tmp && \
    mv /tmp/graalvm-ce-19.2.0 /opt/graalvm && \
